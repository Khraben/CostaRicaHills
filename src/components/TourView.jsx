...
const TourView = () => {
  ...
  const calculateEndDate = (date) => {
    const durationMatch = tour.duration.match(
      /(\d+)\s*(días|dias|día|dia|day|days|horas|hrs)/i
    );
    const durationNumber = durationMatch ? parseInt(durationMatch[1], 10) : 1;
    const durationUnit = durationMatch
    ...
    <TourDetails>
      ...
      <Content>
        ...
        <TourInfo>
          ...
          <p>
            <strong>{i18n.t("tourView.subTitleDestiny")}:</strong>{" "}
            {translatedTour.destination || "Destino no disponible"}
          </p>
          <p>
            <strong>{i18n.t("tourView.subTitleDescription")}:</strong>{" "}
            {translatedTour.description || "Descripción no disponible"}
          </p>
          <p>
            <strong>{i18n.t("tourView.subTitleDuration")}:</strong>{" "}
            {translatedTour.duration || "Duración no disponible"}
          </p>
          <p>
            <strong>{i18n.t("tourView.subTitlePrice")}:</strong>{" "}
            {translatedTour.price || "Precio no disponible"}
          </p>
          <button id="ReservarTour-button" onClick={handleReservationClick}>
            {i18n.t("tourView.buttonReservation")}
          </button>
        </TourInfo>
      </Content>
      <ReviewsSection>
        ...
        <Review>
          <Stars>⭐⭐⭐⭐⭐</Stars>
          <p>
            "¡Una experiencia increíble! El tour fue muy bien organizado y los
            guías fueron muy amables y conocedores."
          </p>
        ...
        <Review>
          <Stars>⭐⭐⭐⭐</Stars>
          <p>
            "Disfruté mucho del tour, los paisajes eran impresionantes y aprendí
            mucho sobre la historia local."
          </p>
      ...
      >
        ...
        <ModalContent isDarkTheme={isDarkTheme}>
          ...
          {endDate && (
            <p>
              {i18n.t("tourView.subTitleEndDate")}:{" "}
        ...
        <ModalFooter>
          <button className="cancel" onClick={handleCancel}>
            {i18n.t("tourView.buttonCancel")}
          </button>
          <button className="confirm" onClick={handleConfirm}>
            {i18n.t("tourView.buttonConfirm")}
          </button>
        </ModalFooter>
      </StyledModal>
      <ToastContainer />
...
const ModalContent = styled.div`
  ...
  .react-datepicker-wrapper {
    ...
    background-color: ${({ isDarkTheme }) =>
      isDarkTheme ? "#333" : "#f9f9f9"};
    color: ${({ isDarkTheme }) =>
      isDarkTheme ? "#fff" : "#000"}; /* Color de texto */
    box-sizing: border-box;
    &::placeholder {
      color: ${({ isDarkTheme }) =>
        isDarkTheme ? "#bbb" : "#888"}; /* Color del placeholder */
    }
    &:focus {
      border-color: ${({ isDarkTheme }) =>
        isDarkTheme ? "#afdb11" : "#759600"};
      outline: none;
    }
  }
`;
